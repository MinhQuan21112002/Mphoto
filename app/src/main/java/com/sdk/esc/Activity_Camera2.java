...
public class Activity_Camera2 extends AppCompatActivity {
    ...
    private void setPrintDialog2(String path) throws Exception {
        ...
        int printerWidth = 578;
        Print.setPrintResolution(203, 203);

        origin=imgSolve.applySharpening(origin,0.8f);
        origin = imgSolve.resizeBitmapWithGPUImage(Activity_Camera2.this,origin, printerWidth);
        origin=imgSolve.applySharpening(origin,0.8f);
        origin=imgSolve.smoothImageWithGPUImage(origin,4.0f);



//        origin=imgSolve.flipBitmap(origin);
        origin = imgSolve.convertToGrayscale2(Activity_Camera2.this,origin);
        Bitmap bmp = (origin);
        ...
        seekBarConst.setProgress(13);
        seekBarLight.setProgress(30);
...
// Chuyển drawable thành bitmap
        ...
        int[] lightValue1 = {10}; // Adjust brightness based on SeekBar progress
        float[] contrastValue = {1.3f};
        Bitmap[] adjustedBitmap2 = {null};
        lightValue1[0] = 10;
        contrastValue[0] = 1.3f;
    ...
    public void printImage(final Bitmap bitmap, final int light, final int size,
        ...
        executorService.execute(() -> {
            ...
            ImageSolve imgSolve=new ImageSolve(Activity_Camera2.this);
            bitmapPrint=imgSolve.adjustContrast(bitmapPrint,1.2f);
    ...
    public void onClickPrint() {
        ...
        try {
            String sText = String.valueOf(counterTime)+"   " +
                    "                                 "+formatted+"     ";
    ...
    private void takePicture() {
        ...
        try {
            ...
            ImageReader reader = ImageReader.newInstance(240,320, ImageFormat.JPEG, 1);
...
// Xác định mức zoom mong muốn (ví dụ: zoomFactor = 2.0 là zoom 2x)
            float zoomFactor = 2f;

// Tính toán vùng cắt cho zoom
            assert sensorRect != null;
            int cropWidth = (int) (sensorRect.width() / zoomFactor);
            int cropHeight = (int) (sensorRect.height() / zoomFactor);
            int cropLeft = (sensorRect.width() - cropWidth) / 2;
            int cropTop = (sensorRect.height() - cropHeight) / 2;

// Tạo vùng cắt (crop region)
            Rect zoomRect = new Rect(cropLeft, cropTop, cropLeft + cropWidth, cropTop + cropHeight);

// Gán vùng cắt vào requestBuilder
            captureBuilder.set(CaptureRequest.SCALER_CROP_REGION, zoomRect);
            // Chế độ ánh sáng ban ngày

// Một số chế độ cân bằng trắng khác:
// CameraMetadata.CONTROL_AWB_MODE_AUTO          // Tự động
// CameraMetadata.CONTROL_AWB_MODE_INCANDESCENT // Đèn sợi đốt
// CameraMetadata.CONTROL_AWB_MODE_FLUORESCENT  // Đèn huỳnh quang
// CameraMetadata.CONTROL_AWB_MODE_WARM_FLUORESCENT // Huỳnh quang ấm
// CameraMetadata.CONTROL_AWB_MODE_DAYLIGHT     // Ánh sáng ban ngày
// CameraMetadata.CONTROL_AWB_MODE_CLOUDY_DAYLIGHT // Ban ngày nhiều mây
// CameraMetadata.CONTROL_AWB_MODE_TWILIGHT    // Ánh sáng hoàng hôn
// CameraMetadata.CONTROL_AWB_MODE_SHADE       // Bóng râm
...
// Đặt bù phơi sáng (Exposure Compensation) - Ví dụ: 0
